document.addEventListener("DOMContentLoaded", () => {
    ...
    function initMainUI() {
        ...
        function showPanel(panelToShow) {
            [roleSelection, reasonSelection, enrollment].forEach(panel => {
    ...
    .then(data => {
        if (data.success) {
            if (data.user_inside) {
                // User is inside - no need for reason, submit immediately (exit flow)
                hiddenEnroll.value = value;
                hiddenRole.value = selectedRole;
                hiddenReason.value = ''; // Empty because no reason on exit
                enrollForm.submit();
            } else {
                // User is outside - show reason selection for entry
                showPanel(reasonSelection);
                step = 3;
            }
        } else {
